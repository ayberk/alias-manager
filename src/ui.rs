...
static BASH_TOKEN_RE: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r#"("[^"]*"|'[^']*'|&&|\|\||[|;]|\s+|-{1,2}[a-zA-Z0-9-]+|\$[a-zA-Z0-9_]+|\$\{[^}]+\}|[^\s;|"'$]+)"#).unwrap()
});
...
pub fn ui(f: &mut Frame, app: &mut App) {
    ...
    {
        let source_name = alias.source_name();
    ...
    let filter_text = if app.focus == Focus::Filter {
...
mod tests {
    ...
    #[test]
    fn test_highlight_bash_command_color() {
        let spans = highlight_bash("ls -la && echo $HOME");
        assert!(!spans.is_empty());
        // First token (command) is green+bold
        assert_eq!(spans[0].style.fg, Some(Color::Green));
    }

    #[test]
    fn test_highlight_bash_token_types() {
        let spans = highlight_bash("git status -v --long $VAR 'quoted'");
        // Find each token by content
        let find = |content: &str| {
            spans
                .iter()
                .find(|s| s.content.trim() == content)
                .map(|s| s.style.fg)
        };
        assert_eq!(find("git"), Some(Some(Color::Green))); // command
        assert_eq!(find("-v"), Some(Some(Color::Yellow))); // short flag
        assert_eq!(find("--long"), Some(Some(Color::Yellow))); // long flag
        assert_eq!(find("$VAR"), Some(Some(Color::Cyan))); // variable
        assert_eq!(find("'quoted'"), Some(Some(Color::LightRed))); // string literal
    }

    #[test]
    fn test_highlight_bash_operators() {
        let spans = highlight_bash("foo && bar || baz | qux");
        let operators: Vec<_> = spans
            .iter()
            .filter(|s| matches!(s.content.trim(), "&&" | "||" | "|"))
            .collect();
        for op in &operators {
            assert_eq!(op.style.fg, Some(Color::Magenta));
        }
        assert_eq!(operators.len(), 3);
    }

    #[test]
    fn test_highlight_bash_empty() {
        assert!(highlight_bash("").is_empty());
    }

    #[test]
    fn test_format_last_used_boundaries() {
        let now = std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
            .unwrap_or_default()
            .as_secs();

        // Zero and future timestamps are "Unknown"
        assert_eq!(format_last_used(0), "Unknown");
        assert_eq!(format_last_used(now + 100), "Unknown");

        // Just now (< 60 s ago)
        assert_eq!(format_last_used(now - 30), "Just now");

        // Singular forms
        assert_eq!(format_last_used(now - 60), "1 minute ago");
        assert_eq!(format_last_used(now - 3600), "1 hour ago");
        assert_eq!(format_last_used(now - 86400), "1 day ago");
        assert_eq!(format_last_used(now - 86400 * 30), "1 month ago");
        assert_eq!(format_last_used(now - 86400 * 365), "1 year ago");

        // Plural forms
        assert_eq!(format_last_used(now - 120), "2 minutes ago");
        assert_eq!(format_last_used(now - 7200), "2 hours ago");
        assert_eq!(format_last_used(now - 86400 * 2), "2 days ago");
    }
}
