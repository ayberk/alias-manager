...
static GIT_SECTION_RE: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"^\[([^\]]+)\]").unwrap());
static GIT_ALIAS_RE: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r"^([^=\s]+)\s*=\s*(.*)").unwrap());

pub fn parse_git_config(path: &Path, aliases: &mut Vec<Alias>) -> io::Result<()> {
    ...
    let section_re = &*GIT_SECTION_RE;
    let alias_re = &*GIT_ALIAS_RE;
...
mod tests {
    ...
    #[test]
    fn test_parse_git_config() -> io::Result<()> {
        let dir = tempdir()?;
